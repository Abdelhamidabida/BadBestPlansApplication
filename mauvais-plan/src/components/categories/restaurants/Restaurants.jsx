...
import { fetchPubrestau, fetchResVille, fetchnombrecp } from '../../services/PublicationService';
import Ajoutcmnt from '../../commentaires/Ajoutcmnt';
...
function Restaurants() {
  ...
  useEffect(() => {
    fetchResVille(contenu3).then(res => {
      setVille(res.data);
    })
    .catch(error => {
      console.log(error);
    ...
    <div className='containerr'>
      ...
      <div className='all' >
        ...
        <div className='Arrr'>
          <div className='motr'>
            <div className='trour'>
              Vous Trouver içi les les mauvais plans des restaurants <br></br>sur toute la tunisie</div>
            <div className='zouza'>
              <div className='nomr'>
                <Form.Group >
                  ...
                  >
                    ...
                    {villes ? villes.map((vil) => <option key={vil._id} value={vil._id}>{vil.nomVille}</option>) : null}
                  </Form.Control>
                </Form.Group>
              </div>
              <Button className='searr' variant="light"> 
                <Link to={"/ville/" + contenu3} className='boutcommm'>
                  Search
                </Link>
              </Button>
            </div>
              ...
              <div className='toar' key={row._id}>
                <Toast>
                  ...
                  <Toast.Body>
                    ...
                    <div className='r2bt'>
                      <button className='boutcomr'>
                        <Link to={"/Ajoutcmnt/" + row._id} className='boutcommm'>
                          <TfiCommentsSmiley style={{ width: '15px', height: '15px', color: '#f5fafc' }} />
                          &nbsp;Commentaire(s) ({nombre[index]})
                        </Link>
                      </button>
                    </div>
                  </Toast.Body>
