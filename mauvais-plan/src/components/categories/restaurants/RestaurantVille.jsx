...
import { fetchResVille, fetchnombrecp } from '../../services/PublicationService';
...
function RestaurantVille() {
  ...
  function sortPublicationsByDateCreated(publications) {
...
// Mettez à jour le composant pour inclure l'indicateur de chargement
useEffect(() => {
  if (id) {
    setIsLoading(true); // Affiche l'indicateur de chargement
    fetchResVille(id)
      .then((res) => {
        const sortedPublications = sortPublicationsByDate(res.data);
        setVille(sortedPublications);
        setIsLoading(false); // Masque l'indicateur de chargement
      })
      .catch((error) => {
        console.log(error);
        setIsLoading(false); // Masque l'indicateur de chargement en cas d'erreur
      });
  }
}, [id]);
  ...
  const records = sortPublicationsByDateCreated(ville).slice(firstIndex, lastIndex);
  ...
  function sortPublicationsByDate(publications) {
    const sortedPublications = [...publications].sort((a, b) => {
      const dateA = new Date(a.createdAt);
      const dateB = new Date(b.createdAt);
      return dateB - dateA;
    ...
    <div>
      ...
      <div className='all'>
        ...
        <div className='Arrr'>
          <div className='motr'>
            <div className='trour'>
              Vous Trouver ici les mauvais plans des restaurants <br></br>sur toute la Tunisie
            </div>
            <div className='zouza'>
              <div className='nomr'>
                <Form.Group>
                  ...
                  >
                    ...
                    {villes ? (
                      villes.map(vil => (
                        <option key={vil._id} value={vil._id}>
                          {vil.nomVille}
                        </option>
                      ))
                    ) : null}
              ...
              <Button className='searr' variant="light">
                <Link to={"/ville/" + contenu3} className='boutcommm' >Search</Link>
              </Button>
            </div>
            {records.map((row, index) => (
              <div className='toar' key={row._id}>
                <Toast>
                  <Toast.Header>
                    <Avatar
                      cursor="pointer"
                    ...
                    <strong className="me-auto">
                      {row.userID?.prenom} {row.userID?.nom}
                    </strong>
                    <small>
                ...
                <div className='toa2'>
                  <Toast>
                    <Toast.Header>
                      <Avatar
                        src={
                          row.entrepriseID?.avatar
                            ? urlimage + row.entrepriseID?.avatar
                            : "https://cdn.pixabay.com/photo/2016/08/08/09/17/avatar-1577909_960_720.png"
                        }
                      />
                      &nbsp; &nbsp;
                      <strong className="me-auto">
                        {row.entrepriseID?.nomen} - {row.ville?.nomVille}
                      </strong>
                      <div>
                        <p>
                          {row.content_en !== "" && (
...
export default RestaurantVille;
